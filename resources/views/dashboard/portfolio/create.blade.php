            ...
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                <div class="md:grid md:grid-cols-3 md:gap-6">
                    ...
                    <div class="mt-5 md:col-span-2 md:mt-0">
                        <form action="{{ route('admin.portfolio.create') }}" method="POST" id="createPostForm" enctype="multipart/form-data">
                            @csrf
                            <div class="shadow sm:overflow-hidden sm:rounded-md">
                                <div class="space-y-6 bg-white px-4 py-5 sm:p-6 grid grid-cols-3 gap-6">
                                    <div class="col-span-3 sm:col-span-2">
                                        {{--<x-form.input  name="title" type="text" id="title" :value="old('title')" />--}}
                                        <label for="title" class="block text-sm font-medium text-gray-700">Title</label>
                                        <div class="mt-1 flex rounded-md shadow-sm">
                                            <input type="text" name="title" id="title" class="block w-full flex-1 rounded-none rounded-r-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="{{ old('title') }}">
                                        </div>
                                        @error('title')
                                ...
                                <div class="space-y-6 bg-white px-4 py-5 sm:p-6">
                                    <div>
                                        ...
                                        <div>
                                            <textarea id="description" name="description" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Description...">{{ old('description') }}</textarea>
                                        </div>
                                        @error('description')
                                            <div class="text-sm text-red-600">{{ $message }}</div>
                                        @enderror
                                ...
                                <div class="space-y-6 bg-white px-4 py-5 sm:p-6">
                                    <div>
                                        <label for="body"  class="block">
                                            ...
                                            <textarea id="body" class="block w-full mt-1 rounded-md" name="body" rows="3">{{ old('body') }}</textarea>
                                        </label>
                                        @error('body')
                                            <div class="text-sm text-red-600">{{ $message }}</div>
                                        @enderror
                                ...
                                <div class="space-y-6 bg-white px-4 py-5 sm:p-6">
                                    <div>
                                        ...
                                        <input class="border border-gray-200 p-2 w-full rounded" name="thumbnail" id="thumbnail" type="file" required>
                                        @error('thumbnail')
                                            <div class="text-sm text-red-600">{{ $message }}</div>
                                        @enderror
                                    </div>
